...
extern const struct char_ops char_special_ops;
extern volatile uint16_t *vga;

extern     void  putch_s(uint16_t port, unsigned char c);
...
extern     void  outportw(uint16_t _port, uint16_t _data);
extern  uint8_t  inportb(uint16_t _port);
extern uint16_t  inportw(uint16_t _port);
extern uint32_t  inportl(uint16_t _port);
extern     void  process_key(void);
//extern  ssize_t  con_read(struct char_dev *dev, char *dest, size_t len)__attribute__((nonnull));
extern  ssize_t  con_write(struct char_dev *dev, const char *src, size_t len)__attribute__((nonnull));
//extern      int  con_init(struct char_dev *dev)__attribute__((nonnull));
extern   struct  dev *add_dev(uint64_t id, uint64_t type, const void *ops, const char *name, void *priv)__attribute__((nonnull(3)));
extern     void *find_dev(uint64_t id, uint64_t type);
extern   struct  dev *find_dev_name(const char *name, uint64_t type)__attribute__((nonnull));
extern     void  ser_status(uint16_t port);
//extern     long  con_pending(struct char_dev *dev)__attribute__((nonnull));
extern uint64_t  read_hpet(void);

extern uint64_t hpet_clk_period;
...
__attribute__((unused)) static inline void insw (unsigned short _port, void *addr, size_t _cnt)
{
	__asm__ volatile (
			"cld ; rep ; insw"
			: "=D" (addr), "=c" (_cnt)
...
__attribute__((unused)) static inline void outw (unsigned short _port, const void *addr, size_t _cnt)
{
	__asm__ volatile (
			"cld ; rep ; outsw"
			: "=S" (addr), "=c" (_cnt)
