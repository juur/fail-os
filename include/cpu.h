...
/* CPUE_PF ->error_code bits */

#define CPUE_PF_PRESENT (1<<0)
#define CPUE_PF_WRITE   (1<<1)
#define CPUE_PF_USER    (1<<2)
#define CPUE_PF_RES     (1<<3)
#define CPUE_PF_INSTR   (1<<4)
#define CPUE_PF_SS      (1<<5)
...
#define APIC_BASE(x) ((uintptr_t)(((x).a.apic_base)<<12))
...
#include <page.h>
extern int printf(const char *format, ...) __attribute__((__format__ (__printf__, 1, 2),nonnull(1)));

__attribute__((nonnull)) static inline void _set_cr3(const pt_t *const pml4, const char *file, const char *func, int line)
{
	//printf("set_cr3: pml4=%p: %s:%s:%d\n", (void *)pml4, file, func, line);
	__asm__ volatile("mov %0, %%cr3;nop"::"r"((uint64_t)pml4):"memory");
}

#define set_cr3(x) _set_cr3((x), __FILE__, __func__, __LINE__)
...
extern void tss_flush(uint16_t tsss);
extern void idt_set_gate64(uint8_t num, uint64_t base, uint16_t sel,   uint8_t dpl, uint8_t ist,  uint8_t type);
extern void gdt_set_gate64(uint8_t num, uint64_t base, uint32_t limit, uint8_t dpl, uint8_t flag, uint8_t type);
extern void gdt_set_gate32(uint8_t num, uint32_t base, uint32_t limit, uint8_t dpl, uint8_t flag, uint8_t type);
extern void print_reg(const struct regs *r)__attribute__((nonnull));
extern void spin_lock(int *lock)__attribute__((nonnull));
extern void spin_unlock(int volatile *lock)__attribute__((nonnull));
